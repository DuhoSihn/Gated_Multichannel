function [ vars, records ] = GM_SNN_simulation_GPU( vars, varargin )
...
if vars.mode == 1
    ...
    init_J_EE_1 = vars.init_J_EE .* idx_J_EE_1;
end
...
dt = gpuArray( dt );
t_memory = gpuArray( t_memory );
t_memory_L = gpuArray( t_memory_L );

tau_E = gpuArray( tau_E );
tau_I = gpuArray( tau_I );
E_L_E = gpuArray( E_L_E );
E_L_I = gpuArray( E_L_I );
Delta_T_E = gpuArray( Delta_T_E );
Delta_T_I = gpuArray( Delta_T_I );
C = gpuArray( C );
E_E = gpuArray( E_E );
E_I = gpuArray( E_I );
V_T = gpuArray( V_T );
A_T = gpuArray( A_T );
tau_T = gpuArray( tau_T );
V_re = gpuArray( V_re );
tau_abs = gpuArray( tau_abs );
a_w = gpuArray( a_w );
b_w = gpuArray( b_w );
...
N_E = gpuArray( N_E );
N_I = gpuArray( N_I );
N_E_all = gpuArray( N_E_all );
N_I_all = gpuArray( N_I_all );
N_E_1 = gpuArray( N_E_1 );
N_E_2 = gpuArray( N_E_2 );
N_I_1 = gpuArray( N_I_1 );
N_I_2 = gpuArray( N_I_2 );
tau_r_E = gpuArray( tau_r_E );
tau_d_E = gpuArray( tau_d_E );
tau_r_I = gpuArray( tau_r_I );
tau_d_I = gpuArray( tau_d_I );
J_EE_min = gpuArray( J_EE_min );
J_EE_max = gpuArray( J_EE_max );
J_EI_min = gpuArray( J_EI_min );
J_EI_max = gpuArray( J_EI_max );
J_ext_EE = gpuArray( J_ext_EE );
J_ext_IE = gpuArray( J_ext_IE );
period_scaling = gpuArray( period_scaling );

A_LTD = gpuArray( A_LTD );
A_LTP = gpuArray( A_LTP );
theta_LTD = gpuArray( theta_LTD );
theta_LTP = gpuArray( theta_LTP );
tau_u = gpuArray( tau_u );
tau_v = gpuArray( tau_v );
tau_x = gpuArray( tau_x );

tau_y = gpuArray( tau_y );
eta = gpuArray( eta );
r_O = gpuArray( r_O );

% triplet_tau_plus = gpuArray( triplet_tau_plus );
% triplet_tau_minus = gpuArray( triplet_tau_minus );
% triplet_tau_x = gpuArray( triplet_tau_x );
% triplet_tau_y = gpuArray( triplet_tau_y );
% triplet_A_2_plus = gpuArray( triplet_A_2_plus );
% triplet_A_3_plus = gpuArray( triplet_A_3_plus );
% triplet_A_2_minus = gpuArray( triplet_A_2_minus );
% triplet_A_3_minus = gpuArray( triplet_A_3_minus );

idx_J_EE = gpuArray( idx_J_EE );
idx_J_EI = gpuArray( idx_J_EI );
idx_J_IE = gpuArray( idx_J_IE );
idx_J_II = gpuArray( idx_J_II );
% idx_J_GII = gpuArray( idx_J_GII );

if vars.mode == 1
    idx_J_EE_1 = gpuArray( idx_J_EE_1 );
    idx_J_EI_1 = gpuArray( idx_J_EI_1 );
    idx_J_IE_1 = gpuArray( idx_J_IE_1 );
    idx_J_II_1 = gpuArray( idx_J_II_1 );
end
...
J_IE = gpuArray( J_IE );
J_II = gpuArray( J_II );
% J_GII = gpuArray( J_GII );

if vars.mode == 1
    J_EE_1 = gpuArray( J_EE_1 );
    J_EI_1 = gpuArray( J_EI_1 );
    J_IE_1 = gpuArray( J_IE_1 );
    J_II_1 = gpuArray( J_II_1 );
end

init_J_EE = gpuArray( init_J_EE );
connects = gpuArray( connects );
idxMat_E = gpuArray( idxMat_E );

if vars.mode == 1
    init_J_EE_1 = gpuArray( init_J_EE_1 );
end

inputs = gpuArray( inputs );
ext_stim_E = gpuArray( ext_stim_E );
ext_stim_I = gpuArray( ext_stim_I );
...
V_T_E = gpuArray( V_T_E );
ww_E = gpuArray( ww_E );

s_ext_EE = gpuArray( s_ext_EE );
s_ext_IE = gpuArray( s_ext_IE );

ct_abs_E = gpuArray( ct_abs_E );
ct_abs_I = gpuArray( ct_abs_I );

% triplet_r_1 = gpuArray( triplet_r_1 );
% triplet_r_2 = gpuArray( triplet_r_2 );
% triplet_o_1 = gpuArray( triplet_o_1 );
% triplet_o_2 = gpuArray( triplet_o_2 );

F_E = gpuArray( F_E );
F_I = gpuArray( F_I );

temp_V_E = gpuArray( temp_V_E );
temp_V_I = gpuArray( temp_V_I );
temp_Vu_E = gpuArray( temp_Vu_E );
temp_Vv_E = gpuArray( temp_Vv_E );

temp_s_E = gpuArray( temp_s_E );
temp_sx_E = gpuArray( temp_sx_E );
temp_sy_E = gpuArray( temp_sy_E );
temp_s_I = gpuArray( temp_s_I );
temp_sy_I = gpuArray( temp_sy_I );

temp_s_ext_EE = gpuArray( temp_s_ext_EE );
temp_s_ext_IE = gpuArray( temp_s_ext_IE );

temp_R_Vu_E = gpuArray( temp_R_Vu_E );
temp_R_V_E = gpuArray( temp_R_V_E );
temp_R_Vv_E = gpuArray( temp_R_Vv_E );

g_EE = gpuArray( g_EE );
g_EI = gpuArray( g_EI );
g_IE = gpuArray( g_IE );
g_II = gpuArray( g_II );
% g_GII = gpuArray( g_GII );

% temp_triplet_r_1 = gpuArray( temp_triplet_r_1 );
% temp_triplet_r_2 = gpuArray( temp_triplet_r_2 );
% temp_triplet_o_1 = gpuArray( temp_triplet_o_1 );
% temp_triplet_o_2 = gpuArray( temp_triplet_o_2 );

% temp_triplet_pre = gpuArray( temp_triplet_pre );
% temp_triplet_post = gpuArray( temp_triplet_post );

if ~isempty( type_records_on )
    records.s_E = gpuArray( records.s_E );
    records.s_I = gpuArray( records.s_I );
end
...
if vars.mode == 1
    J_EE_1 = gather( J_EE_1 );
    J_EI_1 = gather( J_EI_1 );
end
...
ct_abs_E = gather( ct_abs_E );
ct_abs_I = gather( ct_abs_I );

% triplet_r_1 = gather( triplet_r_1 );
% triplet_r_2 = gather( triplet_r_2 );
% triplet_o_1 = gather( triplet_o_1 );
% triplet_o_2 = gather( triplet_o_2 );

if ~isempty( type_records_on )
    records.s_E = gather( records.s_E );
    records.s_I = gather( records.s_I );
end
