function vars = GM_SNN_initiation( vars )
...
% nStimsS = round( vars.infoStims( 5 ) / 2.5 );
nStimsS = round( ( vars.infoStims( 5 ) * ( nChannels / 5 ) ) / 2.5 );
    ...
    for h2 = 1 : length( N_E )
        if vars.connects( h1, h2 ) > 0
            ...
            if h1 ~= h2
                ...
                %                 init_Conn = false( N_E( h1 ), N_E( h2 ) );
                %                 ct4 = 0;
                %                 ct5 = 0;
                %                 for stim = 1 : nStims
                %                     init_Conn( ct4 + [ 1 : ( N_E( h1 ) / nStims ) ], ct5 + [ 1 : ( N_E( h2 ) / nStims ) ] ) = true;
                %                     ct4 = ct4 + ( N_E( h1 ) / nStims );
                %                     ct5 = ct5 + ( N_E( h2 ) / nStims );
                %                 end
                %                 temp_J_EE = J_EE( idxMat_E( h1, : ), idxMat_E( h2, : ) );
                %                 temp_J_EE( init_Conn ) = J_EE_O1;
                %                 J_EE( idxMat_E( h1, : ), idxMat_E( h2, : ) ) = temp_J_EE;
                ...
                %                         init_Conn = circshift( init_Conn, -sStim * ( N_E( h2 ) / nStims ), 2 );
                %                         init_Conn( ct4 + [ 1 : ( N_E( h1 ) / nStims ) ], ct5 + [ 1 : ( N_E( h2 ) / nStims ) ] ) = true;
                %                         init_Conn = circshift( init_Conn, sStim * ( N_E( h2 ) / nStims ), 2 );
                %                     end
                %                     init_Conn( ct4 + [ 1 : ( N_E( h1 ) / nStims ) ], ct5 + [ 1 : ( N_E( h2 ) / nStims ) ] ) = true;
            ...
            elseif h1 == h2
                J_EE( idxMat_E( h1, : ), idxMat_E( h2, : ) ) = J_EE_O3;
                if nChannels > 1
                    for ch = 1 : nChannels
