function vars = GM_stimulation_distProj( vars )
...
% vars = GM_simulation_Hebb( vars, 'plasticity_off' );
vars = GM_simulation_Hebb_GPU( vars, 'plasticity_off' );

% -------------------------------------------------------------------------

vars.projs = cell( vars.infoStims( 4 ), length( vars.areas ) - 1 );
for ch = 1 : vars.infoStims( 4 )
    for h = 1 : length( vars.areas ) - 1
        X = nan( vars.areas( 2 ), nStims * vars.infoStims( 2 ) );
        cumAreas = cumsum( vars.areas );
        for t = 1 : nStims
            X( :, ( t - 1 ) * vars.infoStims( 2 ) + [ 1 : vars.infoStims( 2 ) ] ) = vars.acts( cumAreas( h ) + 1 : cumAreas( h + 1 ), randBegin( ch, t ) + [ 0 : vars.infoStims( 2 ) - 1 ] );
        end
        X = transpose( X );
...
% projection metric
vars.projMetric = nan( vars.infoStims( 4 ), vars.infoStims( 4 ), length( vars.areas ) - 1 );
...
% vars.prodProj = cell( vars.infoStims( 4 ), vars.infoStims( 4 ), length( vars.areas ) - 1 );
% vars.prodProjAll = [];
vars.prodProj = nan( vars.infoStims( 4 ), vars.infoStims( 4 ), length( vars.areas ) - 1 );
for ch1 = 1 : vars.infoStims( 4 ) - 1
    for ch2 = ch1 + 1 : vars.infoStims( 4 )
        for h = 1 : length( vars.areas ) - 1
            proj1 = vars.projs{ ch1, h };
